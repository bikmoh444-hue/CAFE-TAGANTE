...
export default function Statistics() {
  ...
  const getReportExtraExpenses = (report: any) => {
    return Number(report?.extra_expenses || 0);
  };
  ...
  const fetchDailyReport = async () => {
    ...
    try {
      ...
      const dishRev = (reports || []).reduce((acc, r) => {
        return acc + getReportDishTotal(r);
      }, 0);

      const drinksRev = (reports || []).reduce((acc, r) => {
        return acc + getReportDrinksTotal(r);
      }, 0);

      const reportExtraExpenses = (reports || []).reduce((acc, r) => {
        return acc + getReportExtraExpenses(r);
      }, 0);

      const otherRev =
        Number(other?.tyabat_revenue || 0) + Number(other?.chwaya_revenue || 0);
  ...
  const fetchMonthlyReport = async () => {
    ...
    try {
      ...
      const otherRev = (other || []).reduce((acc, r) => {
        return acc + Number(r.tyabat_revenue || 0) + Number(r.chwaya_revenue || 0);
      }, 0);
    ...
    <div className="space-y-8">
        ...
        <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-4">
            {data.reports?.map((report: any) => {
              ...
              const reportDrinksTotal = Number(report.total_drinks_breakfast || 0);
              const reportExtraExpenses = Number(report.extra_expenses || 0);
              const reportGrandTotal = reportDishTotal + reportDrinksTotal;
                ...
                >
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-bold text-gray-900 capitalize">
                      Shift {report.shift} - {report.server_name}
                    </span>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <p className="text-gray-600">
                      ...
                      <span className="font-bold text-amber-600">
                        {formatCurrency(reportDrinksTotal)}
                      </span>
                    </p>
                    <p className="text-gray-600">
                      Plats:{' '}
                      <span className="font-bold text-orange-600">
                        {formatCurrency(reportDishTotal)}
                      </span>
                    </p>
                    <p className="text-gray-600">
                      Dépenses suppl.:{' '}
                      <span className="font-bold text-red-600">
                        {formatCurrency(reportExtraExpenses)}
                      </span>
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-500">
                    ...
                    <p>Tajine Kbir: {report.tajine_kbir_sold} vendus</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <p className="text-sm text-amber-800">
                De <span className="font-bold">{data.handover.source_server}</span> à{' '}
                <span className="font-bold">{data.handover.destination_server}</span>
              </p>
              <p className="text-xs text-amber-600 mt-1">
                Heure: {data.handover.handover_time}
              </p>
              {data.handover.note && (
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                <p className="text-gray-600">
                  ...
                  <span className="font-bold text-gray-900">
                    {data.handover.tajine_sghir_poulet_transmitted}
                  </span>
                </p>
                <p className="text-gray-600">
                  T.S Viande:{' '}
                  <span className="font-bold text-gray-900">
                    {data.handover.tajine_sghir_viande_transmitted}
                  </span>
                </p>
                <p className="text-gray-600">
                  M9la Viande:{' '}
                  <span className="font-bold text-gray-900">
                    {data.handover.m9la_viande_transmitted}
                  </span>
                </p>
                <p className="text-gray-600">
                  Tajine Kbir:{' '}
                  <span className="font-bold text-gray-900">
                    {data.handover.tajine_kbir_transmitted}
                  </span>
                </p>
              </div>
            </div>
      ...
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {data.reportExtraExpenses > 0 && (
            <div className="flex justify-between p-3 border-b border-gray-50 bg-red-50 rounded-lg">
              <div>
                <p className="text-sm font-bold text-gray-900">
                  Dépenses supplémentaires des rapports
                </p>
                <p className="text-xs text-gray-500">Serveurs</p>
              </div>
              <span className="text-sm font-bold text-red-600">
                {formatCurrency(data.reportExtraExpenses)}
              </span>
            </div>
          )}

          {(!data.expenses || data.expenses.length === 0) &&
            !data.reportExtraExpenses && (
              <p className="text-sm text-gray-500 italic">
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <p className="text-2xl font-bold text-gray-900">
            {formatCurrency(data.totalDailyExp)}
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center p-4 bg-red-50 rounded-xl">
              <span className="text-sm font-medium text-red-900">
                Dépenses supplémentaires (rapports)
              </span>
            ...
            <p className="text-xs text-gray-500 italic">
              Les dépenses supplémentaires des rapports sont incluses dans les
              dépenses journalières du mois.
            </p>
          </div>
        </div>
      </div>
