...
import { DailyExpense, ServerReport, OtherRevenue, MonthlyCharge } from '../types';
...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Calculate Today
      const todayServerRev = (serverReportsToday || []).reduce((acc, r) => acc + Number(r.total_drinks_breakfast), 0);
      const todayOtherRev = (otherRevenuesToday?.tyabat_revenue || 0) + (otherRevenuesToday?.chwaya_revenue || 0);
      const todayTotalRev = todayServerRev + todayOtherRev;
      const todayTotalExp = (expensesToday || []).reduce((acc, e) => acc + Number(e.amount), 0);

      // Calculate Month
      const monthServerRev = (serverReportsMonth || []).reduce((acc, r) => acc + Number(r.total_drinks_breakfast), 0);
      const monthOtherRev = (otherRevenuesMonth || []).reduce((acc, r) => acc + Number(r.tyabat_revenue) + Number(r.chwaya_revenue), 0);
      const monthTotalRev = monthServerRev + monthOtherRev;
      const monthTotalExp = (expensesMonth || []).reduce((acc, e) => acc + Number(e.amount), 0);
      const monthTotalCharges = (chargesMonth || []).reduce((acc, c) => acc + Number(c.amount), 0);
      ...
      const chartDataArr = last7Days.map(day => {
        ...
        const rev = dayReports.reduce((acc, r) => acc + Number(r.total_drinks_breakfast), 0) + 
                    (Number(dayOther?.tyabat_revenue) || 0) + (Number(dayOther?.chwaya_revenue) || 0);
        const exp = dayExp.reduce((acc, e) => acc + Number(e.amount), 0);
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {loadingPersonnel ? (
              ...
              <p className="col-span-full text-center text-gray-400 py-4 italic">Aucune présence enregistrée pour aujourd'hui.</p>
            ) : (
              todayAttendance.map((att: any) => (
                <div key={att.id} className="flex flex-col items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                  <div className={cn(
                  ...
                  <p className="text-xs font-bold text-gray-900 text-center truncate w-full">{att.employees?.full_name}</p>
